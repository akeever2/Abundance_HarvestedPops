...
#       estiamted harvest from social surveys (Data/HarvestEstimates.csv), and 
...
tn_harvest_data <- dkmaster %>%
  filter(!(huntyear %in% c(2020, 2021, 2022))) %>%
  select(county, age, sex, huntyear, bag) %>%
  left_join(dmus %>%
              select(NAME, alias, pool), 
            by = c("county" = "NAME"))

# Fix issues in harvest data to keep nomenclature consistent. Also define age
# and sex classes. 
...
# y_sex_statewide[age, yr]
# y_sex_DMU[age, yr, DMU]
...
y_Af_DMU <- tn_harvest_data %>% 
  filter(!is.na(age), bag == "antlerless", sex == 1 | (sex == 2 & age == 1)) %>%
  group_by(huntyear, age, sex, alias) %>%
  summarise(H.total = n()) %>%
  pivot_wider(names_from = huntyear, names_prefix = "yr", values_from = H.total, 
              values_fill = 0) %>%
  ungroup() %>%
  split(.$alias) %>%
  map(~ .x %>% arrange(sex, age) %>% select(-age, -sex, -alias)) %>%
  abind::abind(., along = 3)
...
y_Am_DMU <- tn_harvest_data %>% 
  ...
  ungroup() %>%
  split(.$alias) %>%
  map(~ .x %>% select(-age, -sex, -alias)) %>%
  abind::abind(., along = 3)

# Get total harvest statewide and by DMU
# y_H_statewide[bag, yr]
# y_H_DMU[bag, yr, DMU]
...
y_H_DMU <- tn_harvest_data %>%
  filter(!is.na(alias)) %>%
  group_by(huntyear, bag, alias) %>%
  summarise(H.total = n()) %>%
  arrange(desc(bag)) %>%
  pivot_wider(names_from = bag, values_from = H.total, values_fill = 0) %>%
  ungroup() %>%
  split(.$alias) %>%
  map(~ .x %>% select(-huntyear, -alias)) %>%
  abind::abind(., along = 3)


# Get total number of males and females that were aged and sexed by DMU
O_dmu <- as.data.frame(apply(y_Af_DMU, c(2, 3), sum)) %>% 
  ...
  full_join(as.data.frame(apply(y_Am_DMU, c(2, 3), sum)) %>% 
              rownames_to_column(var = "year") %>% 
              pivot_longer(cols = !contains("year"), names_to = "DMU", 
                           values_to = "mN")) %>% 
  split(.$DMU) %>% 
...
rm(dmus, dkmaster, tn_harvest_data, harvest_ests)
